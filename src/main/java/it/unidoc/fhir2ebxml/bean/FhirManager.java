...
public class FhirManager {
	...
	public void traslateJSON(File in, File out) throws Exception {
		...
		/*
		 * for (HashMap.Entry<String, String> entry : translationMap.entrySet()) {
	...
	private void readSourcePatient(JSONObject oJ, Element root) {
		...
		for (int i = 0; true; i++) {
			try {
				...
				if (!patient.getString("id").equals(sourcePID.substring(1)))
					continue;
	...
	private void readPracticeSettingCode(JSONObject oJ, Element root) {
		JSONObject setting = oJ.getJSONObject("context").getJSONObject("practiceSetting").getJSONArray("coding")
				.getJSONObject(0);
		...
		createClassification(conf.getClassificationSchemePracticeSettingCode(), nodeC, code, name, root, false);
	}

	private void readFacilityTypeCode(JSONObject oJ, Element root) {
		JSONObject facility = oJ.getJSONObject("context").getJSONObject("facilityType").getJSONArray("coding")
				.getJSONObject(0);
		...
		createClassification(conf.getClassificationSchemeFacilityTypeCode(), nodeC, code, name, root, false);
	}
	...
	private void readEventCodeList(JSONObject oJ, Element root) {
		...
		JSONObject event = oJ.getJSONObject("context").getJSONArray("event").getJSONObject(0).getJSONArray("coding")
				.getJSONObject(0);
		...
		createClassification(conf.getClassificationSchemeEventCodeList(), nodeC, code, name, root, false);
	...
	private void readFormatCode(JSONObject oJ, Element root) {
		...
		createClassification(conf.getClassificationSchemeFormatCode(), nodeC, code, name, root, false);
	...
	private void readConfidentialityCode(JSONObject oJ, Element root) {
		...
		createClassification(conf.getClassificationSchemeConfidentialityCode(), nodeC, code, name, root, false);
	...
	private void readPatientID(JSONObject oJ, Element root) {
		...
		createExternalIdentifier(conf.getIdentificationSchemePatienId(), patient.substring(8),
				"XDSDocumentEntry.patientId", root);
	...
	private void readClassCode(JSONObject oJ, Element root) {
		...
		createClassification(conf.getClassificationSchemeClassCode(), nodeC, code, name, root, false);
	}
	...
	private void readTypeCode(JSONObject oJ, Element root) {
		...
		createClassification(conf.getClassificationSchemeTypeCode(), nodeC, code, name, root, false);
	}
	...
	private void readUniqueID(JSONObject oJ, Element root) {
		...
		createExternalIdentifier(conf.getIdentificationSchemeUniqueId(), id, "XDSDocumentEntry.uniqueId", root);
	}
				...
				if (authorReference.contains("#")) {
					...
					for (int j = 0; true; j++) {// Search author's data in contained
						if (!(authorReference.contains(contained.getJSONObject(j).getString("id"))
								&& contained.getJSONObject(j).getString("resourceType").equals("Practitioner")))
							continue;

						aID = contained.getJSONObject(j).getString("id");
						aName = contained.getJSONObject(j).getJSONArray("name").getJSONObject(0).getJSONArray("given")
								.getString(0);
					...
					for (int k = 0; true; k++) {// Search Institution
						try {
							if (!(authorReference.contains(contained.getJSONObject(k).getString("id"))
									&& contained.getJSONObject(k).getString("resourceType").equals("Organization")))
								continue;
							/*
					...
					for (int l = 0; true; l++) {// Search author role and speciality
						...
						if (!(authorReference.contains(contained.getJSONObject(l).getString("id"))
								&& contained.getJSONObject(l).getString("resourceType").equals("PractitionerRole")))
							continue;

						aRole = contained.getJSONObject(l).getJSONArray("code").getJSONObject(0).getJSONArray("coding")
								.getJSONObject(0).getString("display");
						aSpeciality = contained.getJSONObject(l).getJSONArray("specialty").getJSONObject(0)
								.getJSONArray("coding").getJSONObject(0).getString("display");
						break;
					}
	...
	private void addAuthor(String id, String authorPerson, String[] authorInstitution, String aRole, String aSpeciality,
			Element mRoot) {

		// System.out.println("Addauthor");
		Element root = doc.createElement("rim:Classification");
		root.setAttribute("classificationScheme", conf.getClassificationSchemeAuthor());
		root.setAttribute("classifiedObject", entryUUID);
		root.setAttribute("id", id);
		root.setAttribute("objectType", conf.getObjectTypeClassification());
	...
	private void createClassification(String cScheme, String nodeC, String code, String name, Element mRoot,
			boolean author) {

		Element root = doc.createElement("rim:Classification");
		root.setAttribute("classificationScheme", cScheme);
		root.setAttribute("classifiedObject", entryUUID);
	...
	private void createExternalIdentifier(String iScheme, String value, String name, Element mRoot) {
		...
		root.setAttribute("classificationScheme", iScheme);
		root.setAttribute("value", value);
		root.setAttribute("id", "id_x");
		root.setAttribute("objectType", conf.getObjectTypeExternal());
...
//	}
	...
	private Element createExtrinsicObject(JSONObject oJ, Document doc) {
		...
		root.setAttribute("mimeType",
				oJ.getJSONArray("content").getJSONObject(0).getJSONObject("attachment").getString("contentType"));
		root.setAttribute("objectType", conf.getExtrinsicObjectStable());
		doc.appendChild(root);

		return root;
	}

	private void createXML(Document doc, File file) {
